...
#define MORSE_TIMER_BASE 50 // ~= 11 WPM
...
// Morse Letters 'A' - 'Z'
//
// TODO: Put this in PROGRAM space for AVR's.
//
...
void MorseSync::SendLetterInternal(uint8_t letter) {
  ...
  while (atomIndex < 4) {
    ...
    case M_DIT:
      MorseKeyDown(DIT_LENGTH * MORSE_TIMER_BASE);
      delay(INTRALETTER_SPACING * MORSE_TIMER_BASE);
      break;

    case M_DAH:
      MorseKeyDown(DAH_LENGTH * MORSE_TIMER_BASE);
      delay(INTRALETTER_SPACING * MORSE_TIMER_BASE);
      break;
...
//
void MorseSync::MorseKeyDown(int interval) {
 
  if (m_keyer != NULL) {
      m_keyer->KeyDown(interval);
  }
}
